...
### 0.1 Essential Design Documents (Weeks 1-2) | **FOCUS: Planning / Systems**
...
### 0.2 Requirements & Task Analysis | **FOCUS: Planning / Systems**
...
### 0.3 Repository Setup | **FOCUS: DevOps**
...
### 0.4 Development Environment | **FOCUS: DevOps / Autonomy**
...
- [ ] **ENV-03** Install BlueOS v1.2+ on Raspberry Pi 4 with minimal extensions (MAVLink Router, Web UI)
- [ ] **ENV-04** Set up ROS 2 Jazzy on development workstation (Ubuntu 24.04) for simulation/testing
- [ ] **ENV-05** Install Python dependencies: `opencv-python`, `numpy`, `scipy`, `pyyaml`, `pyserial`
- [ ] **ENV-06** Create `setup.bash` script to source ROS 2 and workspace environment variables
...
### 0.5 Simulation Environment | **FOCUS: Autonomy / DevOps**
...
### 0.6 Wiring & Power Diagram | **FOCUS: Hardware**
...
- [ ] **WIR-03** Document pinout assignments for Raspberry Pi, Cube Orange, custom PCB
- [ ] **WIR-04** Validate power budget: calculate total draw vs. battery capacity

#### Acceptance Criteria
- [ ] Wiring diagram is complete and matches the physical build
- [ ] Power budget is validated (total draw < battery capacity with 20% safety margin)
- [ ] Pinout assignments are documented and version-controlled

---

### 0.7 Test Plan | **FOCUS: Planning / Systems**
...
### 0.8 Hardware Abstraction Layer (Cube Orange) | **FOCUS: Control / Hardware**
...
- [ ] **HAL-09** Conduct benchtop thrust-vs-PWM characterization for all 8 thrusters (critical for dead reckoning model) → **DEPENDENCY for AUTO-04**
...
### 0.9 Hydrophone Hardware Integration | **FOCUS: Hardware**
...
### 0.10 Hydrophone Signal Processing | **FOCUS: Perception**
...
### 0.11 Control Layer (Raspberry Pi - ROV Mode) | **FOCUS: Control / DevOps**

**Goal:** Set up the companion computer for MAVLink routing, I/O, and manual control.

- [ ] **CTRL-01** Install BlueOS v1.2+ on Raspberry Pi 4 with minimal extensions (MAVLink Router, Web UI)
...
- [ ] **CTRL-05** Add basic fail-safe: if Raspberry Pi loses Jetson heartbeat for >5s, surface command
...
- [ ] Fail-safe triggers surface command when Jetson heartbeat stops (tested on bench) within 5s

---

### 0.12 Operator Interface (ROV Dashboard) | **FOCUS: DevOps / Autonomy**
...
- [ ] **UI-02** Configure dashboard to display: primary camera feed (OAK-D Lite), depth, heading, thruster status, leak detection
...
- [ ] **UI-05** Validate interface usability: single-screen operation without window switching
...
### 0.13 Hardware Bring-Up (Physical Vehicle) | **FOCUS: Hardware**
...
- [ ] **HW-04.1** Measure actual power consumption: idle, full thrust, and nominal operation
...
- [ ] **HW-10** Mount and calibrate CTD sensor (if available): verify temperature, conductivity, and depth readings
...
- [ ] Power consumption measured and documented (idle, nominal, full thrust)
...
- [ ] CTD sensor returns valid temperature, conductivity, and depth readings

---

### 0.14 ROV Integration & Sea Trials (Without Jetson) | **FOCUS: Control / Hardware**
...
### 0.15 Jetson Integration | **FOCUS: Perception / DevOps**
...
- [ ] **JET-04** Verify OAK-D Lite and ArduCam B0178 streams over USB 3.0; check for dropped frames
- [ ] **JET-05** (Optional) Run visual odometry pipeline in passive mode — no control outputs
- [ ] **JET-06** Test fail-safe on bench: disconnect Jetson USB/UART → Raspberry Pi commands surface within 5s
...
- [ ] OAK-D Lite and ArduCam B0178 streams are visible in `rviz2` or `rqt_image_view` with < 5% dropped frames
- [ ] Fail-safe triggers surface command within 5s of Jetson heartbeat loss on bench
...
| 4 | All sensors (Bar30, IMU, leak, temp, humidity, CTD) return valid data | Log review |
| 5 | All 8 thrusters characterized (PWM → thrust curve) | Benchtop test documentation |
| 6 | OAK-D Lite and ArduCam B0178 stream without frame drops > 5% | Log review |
...
### 1.0 Pre-Phase 1 Planning (Week 9)
...
### 1.0.1 Development Environment (Phase 1 Additions) (Weeks 9-10)
...
- [ ] **ENV-AUV-07** Write `Dockerfile.pi` with BlueOS v1.2+ extension development environment
...
### 1.1 Perception Layer (Weeks 10-13)

**Goal:** Implement visual odometry, sensor fusion, and quality monitoring.

- [ ] **PER-01** Install OAK-D Lite drivers (`depthai-ros`) on Jetson; verify camera streaming at 30Hz for SLAM
- [ ] **PER-02** Install ArduCam B0178 drivers; verify global shutter streaming at 60Hz for optical flow
- [ ] **PER-03** Implement ORB-SLAM3 (stereo mode) with ROS 2 Jazzy wrapper; validate against simulation
- [ ] **PER-04** Implement KLT optical flow on ArduCam B0178 as fallback odometry
...
#### Acceptance Criteria
- [ ] OAK-D Lite publishes stereo images at 30Hz; ArduCam B0178 publishes at 60Hz
...
### 1.1.1 Acoustic Localization Integration (Weeks 11-12) | **FOCUS: Perception**
...
- [ ] **PER-14** Implement acoustic dropout detection: handle cases where no pinger is detected
...
- [ ] Acoustic dropout detection triggers fallback behavior when no pinger detected

---

### 1.2 Autonomy Layer (Core) (Weeks 13-16)
...
### 1.2.1 Acoustic Navigation Behaviors (Weeks 14-15) | **FOCUS: Autonomy**
...
### 1.3 Control Layer (AUV Enhancements) (Weeks 15-16)
...
### 1.4 HAL Enhancements (Cube Orange) (Weeks 16-17)

**Goal:** Configure EKF3 to accept external vision data conditionally.

- [ ] **HAL-AUV-01** Configure EKF3 to accept external vision via `VISION_POSITION_ESTIMATE` MAVLink messages → **DEPENDENCY: CTRL-AUV must be functional**
...
### 1.5 Perception Hardware Installation (Weeks 16-17)
...
- [ ] **HW-AUV-02** Mount ArduCam B0178 Global Shutter in separate housing; verify waterproofing
- [ ] **HW-AUV-03** Install humidity sensor inside dome; route I2C to Raspberry Pi
- [ ] **HW-AUV-04** Configure camera exposures: OAK-D Lite (auto-exposure), ArduCam B0178 (fixed exposure for optical flow)
...
- [ ] OAK-D Lite streams at 30Hz, ArduCam B0178 at 60Hz with < 5% dropped frames
...
### 1.6 AUV Integration & Pool Tests (Weeks 18-19)
...
- [ ] **INT-AUV-07** Pool test: acoustic + visual sensor fusion - verify acoustic tracking during visual dropout
- [ ] **INT-AUV-08** **[DEFER]** Full competition run (all tasks) → defer until Phase 2
...
- [ ] Acoustic tracking maintains position estimate within 1.0m during 10s visual dropout (pool test)

---

### 1.7 Pre-Competition Validation (Week 20)
...
| 6 | Acoustic localization fuses with visual odometry and improves position estimate during dropout | Pool test with visual dropout |
| 7 | All parameters are documented and version-controlled | File review |
| 8 | Docker/CI/CD pipeline passes on `main` branch | CI/CD status check |
| 9 | Team can transition to Phase 2 (competition-specific tasks) with confidence | Team consensus |

**Phase 1 Duration:** Weeks 9-20  
**Phase 1 Go/No-Go Decision:** All 9 criteria must be marked ✅ before proceeding to Phase 2.

---

## Phase 2: Competition-Ready (Weeks 21-24)

### 2.1 ML & Perception Upgrades (Weeks 21-22)
...
- [ ] **ML-04** **[DEFER]** Underwater image enhancement (auto white balance, contrast correction) → if time permits; otherwise document limitations
...
- [ ] If ML-04 not completed, document image quality limitations and manual white balance settings

---

### 2.2 Advanced Autonomy (Weeks 22-23)
...
### 2.3 Competition-Specific Tasks (Weeks 23-24)
...
### 2.4 Finals Preparation (Week 24)
...
**To be completed 48 hours before competition:**

- [ ] All systems power on and communicate (Jetson, Pi, Cube Orange)
- [ ] All sensors return valid data (IMU, depth, cameras, hydrophones)
- [ ] All thrusters spin in correct direction
- [ ] Emergency kill switch functions correctly
- [ ] Mission script loads without errors
- [ ] Battery charged and spare batteries available
- [ ] Recovery kit packed (tools, spare parts, waterproof containers)
- [ ] All team members have reviewed troubleshooting checklist
...
| **Phase 1 Complete** | 20 | All 9 Phase 1 criteria met → AUV with core autonomy |
| **Phase 2 Complete** | 24 | All 8 Phase 2 criteria met → Competition-ready AUV |
| **Final Readiness** | 24-25 | All final readiness checks completed |
...
| **Hydrophone array limitations** | Acoustic localization accuracy depends on array geometry and SNR | Calibrate array carefully (HYD-HW-04); use TDOA with multiple frequency bands; implement dropout detection (PER-14) |
...
4. **Calibration is Critical:** The tasks (HAL-09, HAL-10, HW-09, HYD-HW-04) are essential for dead reckoning and sensor accuracy. Don't skip them.
...
9. **Acoustic Integration Timeline:** Acoustic tasks (PER-11 to PER-14, AUTO-11 to AUTO-14) should be prioritized early in Phase 1 (Weeks 11-15) to allow sufficient time for pool testing and integration.

10. **Servo Control Calibration:** Validate servo PWM ranges (TASK-03, TASK-04) during Phase 0 hardware bring-up (HW-03) to avoid last-minute issues.